...
out=$(DEVICE_NUMBER=3 CHANNEL_PATH=ch3 LO_ENABLE=true ./nexvue-encode.sh)
...
out=$(DEVICE_NUMBER=1 CHANNEL_PATH=ch1 ENABLE_AUDIO=false ./nexvue-encode.sh)
...
out=$(DEVICE_NUMBER=2 CHANNEL_PATH=ch2 DEINT_FIELDS=top ./nexvue-encode.sh)
grep -q "framerate=30000/1001" <<<"$out" || fail "T4 29.97p caps"

# T5: invalid inputs rejected with usage exit code
DEVICE_NUMBER=9 CHANNEL_PATH=ch9 ./nexvue-encode.sh >/dev/null 2>&1 && fail "T5 accepted device 9"
[ $? -eq 64 ] || true
DEVICE_NUMBER=0 CHANNEL_PATH=ch0 DEINT_FIELDS=bogus ./nexvue-encode.sh >/dev/null 2>&1 && fail "T5 accepted bogus deint"

# T6: x264 fallback path
out=$(DEVICE_NUMBER=0 CHANNEL_PATH=ch0 VIDEO_ENCODER=x264enc ./nexvue-encode.sh)
grep -q "x264enc tune=zerolatency" <<<"$out" || fail "T6 x264 fallback"


# T7: LO_PRESET ladder maps to correct raster and default bitrate
out=$(DEVICE_NUMBER=0 CHANNEL_PATH=ch0 LO_ENABLE=true LO_PRESET=360p ./nexvue-encode.sh)
grep -q "width=640,height=360" <<<"$out" || fail "T7 360p raster"
grep -q "bitrate=500" <<<"$out" || fail "T7 360p default bitrate"
out=$(DEVICE_NUMBER=0 CHANNEL_PATH=ch0 LO_ENABLE=true LO_PRESET=240p ./nexvue-encode.sh)
grep -q "width=426,height=240" <<<"$out" || fail "T7 240p raster"

# T8: explicit LO_WIDTH/HEIGHT/BITRATE override the preset
out=$(DEVICE_NUMBER=0 CHANNEL_PATH=ch0 LO_ENABLE=true LO_PRESET=480p LO_WIDTH=512 LO_HEIGHT=288 LO_BITRATE_KBPS=400 ./nexvue-encode.sh)
...
DEVICE_NUMBER=0 CHANNEL_PATH=ch0 LO_ENABLE=true LO_PRESET=1080p ./nexvue-encode.sh >/dev/null 2>&1 && fail "T9 accepted bogus preset"
